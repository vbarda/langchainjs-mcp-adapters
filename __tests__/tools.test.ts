  ...
  describe("loadMcpTools", () => {
    test("should load all tools from client", async () => {
      ...
      expect(tools[0].name).toBe(
        "mcp__mockServer(should load all tools)__tool1"
      );
      expect(tools[1].name).toBe(
        "mcp__mockServer(should load all tools)__tool2"
      );
    });
    ...
    test("should filter out tools without names", async () => {
      ...
      expect(tools.length).toBe(2);
      expect(tools[0].name).toBe(
        "mcp__mockServer(should filter out tools without names)__tool1"
      );
      expect(tools[1].name).toBe(
        "mcp__mockServer(should filter out tools without names)__tool2"
      );
    });
